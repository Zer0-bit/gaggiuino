...
ShotSnapshot buildShotSnapshot(uint32_t timeInShot, SensorState& state,
                               CurrentPhase& phase) {
  float targetFlow = (phase.getType() == PHASE_TYPE::PHASE_TYPE_FLOW)
                         ? phase.getTarget()
                         : phase.getRestriction();
  float targetPressure = (phase.getType() == PHASE_TYPE::PHASE_TYPE_PRESSURE)
                             ? phase.getTarget()
                             : phase.getRestriction();

  return ShotSnapshot{.timeInShot = timeInShot,
...
float Phase::getTarget(uint32_t timeInPhase) const {
  long transitionTime =
      fmax(0L, target.time > 0L ? target.time : stopConditions.time);
  return mapRange(timeInPhase, 0.f, transitionTime, target.start, target.end, 1,
                  target.curve);
}
...
bool Phase::isStopConditionReached(SensorState& currentState,
                                   uint32_t timeInShot,
                                   ShotSnapshot stateAtPhaseStart) const {
...
//-------------------------- StopConditions ----------------------------//
//----------------------------------------------------------------------//
bool PhaseStopConditions::isReached(SensorState& state, long timeInShot,
                                    ShotSnapshot stateAtPhaseStart) const {
  float flow = state.weight > 0.4f ? state.weightFlow : state.smoothedPumpFlow;
  float stopDelta = flow * (state.shotWeight / 100.f);

  return (time >= 0L &&
          timeInShot - stateAtPhaseStart.timeInShot >= (uint32_t)time) ||
         ...
         (waterPumpedInPhase > 0.f &&
          state.waterPumped - stateAtPhaseStart.waterPumped >
              waterPumpedInPhase - stopDelta) ||
...
//--------------------------- CurrentPhase -----------------------------//
//----------------------------------------------------------------------//
CurrentPhase::CurrentPhase(int index, const Phase& phase, uint32_t timeInPhase)
...
CurrentPhase::CurrentPhase(const CurrentPhase& currentPhase)
    : index(currentPhase.index),
      ...
      timeInPhase(currentPhase.timeInPhase) {}
...
int CurrentPhase::getIndex() {
  return index;
}

long CurrentPhase::getTimeInPhase() {
  return timeInPhase;
}
...
void PhaseProfiler::updatePhase(uint32_t timeInShot, SensorState& state) {
  ...
  if (phaseIdx >= profile.phaseCount() ||
      profile.globalStopConditions.isReached(state, timeInShot)) {
  ...
  if (!profile.phases[phaseIdx].isStopConditionReached(state, timeInShot,
                                                       phaseChangedSnapshot)) {
    currentPhase.update(phaseIdx, profile.phases[phaseIdx], timeInPhase);
    return;
  }
