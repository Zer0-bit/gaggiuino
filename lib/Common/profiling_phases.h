...
enum class PHASE_TYPE { PHASE_TYPE_FLOW, PHASE_TYPE_PRESSURE };
...
struct Transition {
  ...
  Transition(float start, float end,
             TransitionCurve curve = TransitionCurve::LINEAR, long time = 0)
      : start(start), end(end), curve(curve), time(time) {}
  Transition(float value)
      : start(value), end(value), curve(TransitionCurve::INSTANT), time(0) {}
};
...
struct Phase {
  ...
  bool isStopConditionReached(SensorState& currentState, uint32_t timeInShot,
                              ShotSnapshot stateAtPhaseStart) const;
};
...
struct Profile {
  ...
  size_t phaseCount() { return phases.size(); }

  void addPhase(Phase phase) { phases.push_back(phase); }
...
class PhaseProfiler {
 private:
  ...
  ShotSnapshot phaseChangedSnapshot = ShotSnapshot{
      0, 0, 0,
      0, 0, 0};  // State when the profiler move to this currentPhaseIdx
