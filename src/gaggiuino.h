...
#define GET_KTYPE_READ_EVERY \
  70  // thermocouple data read interval not recommended to be changed to lower than 250 (ms)
#else
#define GET_KTYPE_READ_EVERY \
  250  // thermocouple data read interval not recommended to be changed to lower than 250 (ms)
#endif
...
#define BOILER_FILL_START_TIME \
  3000UL  //Start boiler fill after 3 seconds since startup, gives system time to fully init.
#define BOILER_FILL_TIMEOUT \
  8000UL  //Stop boiler fill if it's been 8 seconds since system init and fill conditions aren't still met.
