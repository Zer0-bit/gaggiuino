...
class PredictiveWeight {
 ...
 public:
  ...
  bool isOutputFlow() { return outputFlowStarted; }

  void update(const SensorState& state, CurrentPhase& phase,
              const eepromValues_t& cfg) {
    ...
    puckResistance = pressure * 1000.f /
                     state.smoothedPumpFlow;  // Resistance in mBar * s / g
    ...
    if (phaseTypePressure &&
        (resistanceDelta > 500.f || puckResistance < 1100.f)) {
      return;
    }
  ...
  void setIsForceStarted(bool value) { isForceStarted = value; }
