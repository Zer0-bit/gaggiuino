...
inline static float TEMP_DELTA(float d, const SensorState& currentState) {
  return (d * (currentState.pumpFlow < 1.f ? currentState.pumpFlow / 9.f
                                           : currentState.pumpFlow / 10.f));
}

void justDoCoffee(eepromValues_t& runningCfg, SensorState& currentState,
                  bool brewActive, bool preinfusionFinished) {
  ...
  int HPWR_OUT = mapRange(sensorTemperature, runningCfg.setpoint - 10,
                          runningCfg.setpoint, runningCfg.hpwr, HPWR_LOW, 0);
  HPWR_OUT = constrain(
      HPWR_OUT, HPWR_LOW,
      runningCfg.hpwr);  // limits range of sensor values to HPWR_LOW and HPWR
  BREW_TEMP_DELTA = mapRange(
      sensorTemperature, runningCfg.setpoint,
      runningCfg.setpoint + TEMP_DELTA(runningCfg.setpoint, currentState),
      TEMP_DELTA(runningCfg.setpoint, currentState), 0, 0);
  BREW_TEMP_DELTA = constrain(BREW_TEMP_DELTA, 0,
                              TEMP_DELTA(runningCfg.setpoint, currentState));
  ...
  if (brewActive) {  //if brewState == true
    if (sensorTemperature <= runningCfg.setpoint) {
      setBoilerOn();
    } else if (sensorTemperature <=
               runningCfg.setpoint +
                   (runningCfg.brewDeltaState ? BREW_TEMP_DELTA : 0.f)) {
      pulseHeaters(runningCfg.hpwr, runningCfg.mainDivider,
                   runningCfg.brewDivider, brewActive);
    } else {
      setBoilerOff();
    }
    ...
    if (sensorTemperature <= ((float)runningCfg.setpoint - 10.00f)) {
      ...
      pulseHeaters(HPWR_OUT, runningCfg.brewDivider, runningCfg.brewDivider,
                   brewActive);
    } else {
      setBoilerOff();
    }
...
void pulseHeaters(uint32_t pulseLength, int factor_1, int factor_2,
                  bool brewActive) {
...
void steamCtrl(const eepromValues_t& runningCfg, SensorState& currentState,
  ...
  if ((currentState.smoothedPressure <= 9.f) &&
    ...
    (currentState.smoothedPressure < 1.8f) ? setPumpToRawValue(3)
                                           : setPumpOff();
#else
    setPumpOff();
#endif
  ...
  if (currentState.smoothedPressure > 3.f) {
    ...
    (steamTimeout >= STEAM_TIMEOUT) ? currentState.isSteamForgottenON = true
