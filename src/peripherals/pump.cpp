...
float getPumpPct(float targetPressure, float flowRestriction,
  ...
  float maxPumpPct = flowRestriction <= 0.f
                         ? 1.f
                         : getClicksPerSecondForFlow(
                               flowRestriction, currentState.smoothedPressure) /
                               (float)maxPumpClicksPerSecond;
  float pumpPctToMaintainFlow =
      getClicksPerSecondForFlow(currentState.smoothedPumpFlow,
  ...
  if (diff > 0.f) {
    return fminf(maxPumpPct,
                 pumpPctToMaintainFlow * 0.95f + 0.1f + 0.2f * diff);
  }
...
// - flow
// - pressure direction
void setPumpPressure(float targetPressure, float flowRestriction,
                     SensorState& currentState) {
...
float getPumpFlowPerClick(float pressure) {
  float fpc = (flowPerClickAtZeroBar + pressureInefficiencyConstant0) +
              (pressureInefficiencyConstant1 +
               (pressureInefficiencyConstant2 +
                (pressureInefficiencyConstant3 +
                 (pressureInefficiencyConstant4 +
                  (pressureInefficiencyConstant5 +
                   pressureInefficiencyConstant6 * pressure) *
                      pressure) *
                     pressure) *
                    pressure) *
                   pressure) *
...
void setPumpFlow(float targetFlow, float pressureRestriction,
                 SensorState& currentState) {
  ...
  if (pressureRestriction > 0) {
    ...
    float pumpPct =
        getClicksPerSecondForFlow(targetFlow, currentState.smoothedPressure) /
        (float)maxPumpClicksPerSecond;
