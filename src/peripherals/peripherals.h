...
static inline bool brewState(
    void) {  //Monitors the current flowing through the ACS712 circuit and returns a value depending on the power value (P) the system draws
...
static inline bool waterState(void) {
  return waterPinState() ||
         (steamState() &&
          brewState());  // use either an actual switch, or the GC/GCP switch combo
}
