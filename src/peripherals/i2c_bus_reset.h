...
int I2C_ClearBus(int sdaPin, int sclPin) {
  ...
  TWCR &= ~(_BV(
      TWEN));  //Disable the Atmel 2-Wire interface so we can control the sdaPin and sclPin pins directly
#endif
  pinMode(
      sdaPin,
  ...
  while (SDA_LOW && (clockCount > 0)) {  //  vii. If sdaPin is Low,
    ...
    while (SCL_LOW &&
           (counter >
            0)) {  //  loop waiting for sclPin to become High only wait 2sec.
      counter--;
      delay(100);
    ...
    SDA_LOW = (digitalRead(sdaPin) ==
               LOW);  //   and check sdaPin input again and loop
  }
