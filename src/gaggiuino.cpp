...
void setup(void) {
  ...
  pumpInit(eepromCurrentValues.powerLineFrequency,
           eepromCurrentValues.pumpFlowAtZero);
  LOG_INFO("Pump init");
  ...
  if (scalesIsPresent() && millis() > scalesTimer) {
    if (!tareDone) {
      ...
      if (!nonBrewModeActive &&
          (scalesGetWeight() < -0.3f || scalesGetWeight() > 0.3f))
        tareDone = false;
      else
        tareDone = true;
    }
...
void sensorsReadPressure(void) {
  if (millis() > pressureTimer) {
    ...
    currentState.isPressureRisingFast =
        currentState.smoothedPressure >= previousSmoothedPressure + 1.55f;
    currentState.isPressureFalling =
        currentState.smoothedPressure <= previousSmoothedPressure - 0.05f;
    currentState.isPressureFallingFast =
        currentState.smoothedPressure <= previousSmoothedPressure - 0.1f;
...
void calculateWeightAndFlow() {
  ...
  if (brewActive) {
    ...
    if (elapsedTime > REFRESH_FLOW_EVERY) {
      ...
      currentState.isPumpFlowRisingFast =
          currentState.smoothedPumpFlow > previousSmoothedPumpFlow + 2.5f;
      currentState.isPumpFlowFallingFast =
          currentState.smoothedPumpFlow < previousSmoothedPumpFlow - 0.55f;
      ...
      currentState.waterPumped +=
          currentState.smoothedPumpFlow * (float)elapsedTime / 1000.f;
    }
  } else {
    if (elapsedTime > REFRESH_FLOW_EVERY) {
      flowTimer = millis();
...
void pageValuesRefresh(bool forcedUpdate) {
  ...
  if (lcdCurrentPageId != lcdLastCurrentPageId || forcedUpdate == true) {
    ...
    selectedOperationalMode = (OPERATION_MODES)
        lcdGetSelectedOperationalMode();  // MODE_SELECT should always be LAST
...
/*LCD pressure output, as a measure to beautify the graphs locking the live pressure read for the LCD alone*/
    ...
    /*LCD weight output*/
    if (static_cast<SCREEN_MODES>(lcdCurrentPageId) ==
            SCREEN_MODES::SCREEN_home &&
        homeScreenScalesEnabled) {
      lcdSetWeight(currentState.weight);
    } else if (static_cast<SCREEN_MODES>(lcdCurrentPageId) ==
                   SCREEN_MODES::SCREEN_brew_graph ||
               static_cast<SCREEN_MODES>(lcdCurrentPageId) ==
                   SCREEN_MODES::SCREEN_brew_manual) {
    ...
    if (static_cast<SCREEN_MODES>(lcdCurrentPageId) ==
      ...
      lcdSetFlow(
          currentState.weight >
                  0.4f  // currentState.weight is always zero if scales are not present
...
void lcdTrigger1(void) {
  ...
  switch (static_cast<SCREEN_MODES>(lcdCurrentPageId)) {
    case SCREEN_MODES::SCREEN_brew_settings:
      ...
      eepromCurrentValues.switchPhaseOnThreshold =
          lcdValues.switchPhaseOnThreshold;
      break;
    case SCREEN_MODES::SCREEN_profiles:
      // PRESSURE PARAMS
      eepromCurrentValues.pressureProfilingStart =
          lcdValues.pressureProfilingStart;
      eepromCurrentValues.pressureProfilingFinish =
          lcdValues.pressureProfilingFinish;
      eepromCurrentValues.pressureProfilingHold =
          lcdValues.pressureProfilingHold;
      eepromCurrentValues.pressureProfilingLength =
          lcdValues.pressureProfilingLength;
      eepromCurrentValues.pressureProfilingState =
          lcdValues.pressureProfilingState;
      ...
      eepromCurrentValues.preinfusionFlowSoakTime =
          lcdValues.preinfusionFlowSoakTime;
      eepromCurrentValues.preinfusionFlowPressureTarget =
          lcdValues.preinfusionFlowPressureTarget;
      ...
      eepromCurrentValues.flowProfilePressureTarget =
          lcdValues.flowProfilePressureTarget;
      eepromCurrentValues.flowProfileCurveSpeed =
          lcdValues.flowProfileCurveSpeed;
      break;
    ...
    case SCREEN_MODES::SCREEN_shot_settings:
      ...
      eepromCurrentValues.shotStopOnCustomWeight =
          lcdValues.shotStopOnCustomWeight;
      break;
    default:
      break;
  }
...
void updateProfilerPhases() {
  ...
  if (runningCfg.preinfusionState) {
    if (runningCfg.preinfusionFlowState) {  // flow based PI enabled
      float stopOnPressureAbove = (runningCfg.switchPhaseOnThreshold)
                                      ? runningCfg.preinfusionFlowPressureTarget
                                      : -1;
      addFlowPhase(Transition{runningCfg.preinfusionFlowVol},
                   runningCfg.preinfusionFlowPressureTarget,
                   runningCfg.preinfusionFlowTime * 1000, stopOnPressureAbove);
      addFlowPhase(Transition{0.f}, 0,
                   runningCfg.preinfusionFlowSoakTime * 1000, -1);
      preInfusionFinishBar =
          fmaxf(0.f, runningCfg.preinfusionFlowPressureTarget);
    } else {  // pressure based PI enabled
      float stopOnPressureAbove =
          (runningCfg.switchPhaseOnThreshold) ? runningCfg.preinfusionBar : -1;
      addPressurePhase(Transition{(float)runningCfg.preinfusionBar}, 4.5f,
                       runningCfg.preinfusionSec * 1000, stopOnPressureAbove);
      addPressurePhase(Transition{0.f}, -1, runningCfg.preinfusionSoak * 1000,
                       -1);
  ...
  if (runningCfg.pressureProfilingState) {
    if (runningCfg.flowProfileState) {  // flow based profiling enabled
      addFlowPhase(
          Transition{runningCfg.flowProfileStart, runningCfg.flowProfileEnd,
                     TransitionCurve::LINEAR,
                     runningCfg.flowProfileCurveSpeed * 1000},
          runningCfg.flowProfilePressureTarget, -1, -1);
      ...
      uint16_t rampAndHold =
          runningCfg.preinfusionRamp + runningCfg.pressureProfilingHold;
      addPressurePhase(
          Transition{preInfusionFinishBar, ppStart, TransitionCurve::EASE_OUT,
                     runningCfg.preinfusionRamp * 1000},
          -1, rampAndHold * 1000, -1);
      addPressurePhase(Transition{ppStart, ppEnd, TransitionCurve::EASE_IN_OUT,
                                  runningCfg.pressureProfilingLength * 1000},
                       -1, -1, -1);
    }
  } else {  // Shot profiling disabled. Default to 9 bars
    addPressurePhase(
        Transition{preInfusionFinishBar, 9, TransitionCurve::EASE_OUT,
                   runningCfg.preinfusionRamp * 1000},
        -1, -1, -1);
  }
}
...
void addPhase(PHASE_TYPE type, Transition target, float restriction, int timeMs,
  ...
  profile.addPhase(Phase{.type = type,
                         ...
                         .stopConditions = PhaseStopConditions{
                             .time = timeMs, .pressureAbove = pressureAbove}});
}
...
void profiling() {
  if (brewActive) {  //runs this only when brew button activated and pressure profile selected
    ...
    preinfusionFinished =
        currentPhase.getIndex() >= preInfusionFinishedPhaseIdx;
...
void systemHealthCheck(float pressureThreshold) {
  ...
  while (currentState.temperature <= 0.0f || currentState.temperature == NAN ||
    ...
    if (millis() > thermoTimer) {
      ...
      steamState() ? lcdShowPopup("COOLDOWN")
...
bool isBoilerFillPhase(unsigned long elapsedTime) {
  return static_cast<SCREEN_MODES>(lcdCurrentPageId) ==
             SCREEN_MODES::SCREEN_home &&
...
bool isSwitchOn() {
  return brewState() && static_cast<SCREEN_MODES>(lcdCurrentPageId) ==
                            SCREEN_MODES::SCREEN_home;
}
