...
#define EEPROM_METADATA_LOADER(__eepromDataVersion, __eepromMetadata_tName, \
                               __upgradeSchema_fName)                       \
  (eepromValues_t & targetValues) {                                         \
    ...
    if (eepromMetadata.version != __eepromDataVersion ||                    \
        eepromMetadata.versionTimestampXOR != XOR) {                        \
      return false;                                                         \
    }                                                                       \
...
#define REGISTER_LEGACY_EEPROM_DATA(version, eepromData_tName,             \
                                    upgradeSchema_fName)                   \
  /* declare & define metadata */                                          \
  struct _eepromMetadata_t_v##version EEPROM_METADATA_T(eepromData_tName); \
  /* define loader function */                                             \
  static bool _loadEepromMetadata_v##version EEPROM_METADATA_LOADER(       \
      version, _eepromMetadata_t_v##version, upgradeSchema_fName);         \
  /* register loader ptr in array */                                       \
  void __attribute__((constructor))                                        \
  _registerEepromMetadataLoader_v##version() {                             \
