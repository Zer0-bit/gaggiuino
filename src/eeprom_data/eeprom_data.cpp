...
static bool loadCurrentEepromData EEPROM_METADATA_LOADER(EEPROM_DATA_VERSION,
                                                         eepromMetadata_t,
                                                         copy_t);
...
bool eepromWrite(eepromValues_t eepromValuesNew) {
  ...
  */
  ...
  if (eepromValuesNew.preinfusionState != 0 &&
      eepromValuesNew.preinfusionState != 1) {
    LOG_ERROR(errMsg);
    return false;
  }

  if (eepromValuesNew.pressureProfilingState != 0 &&
      eepromValuesNew.pressureProfilingState != 1) {
    LOG_ERROR(errMsg);
    return false;
  }

  if (eepromValuesNew.preinfusionFlowState != 0 &&
      eepromValuesNew.preinfusionFlowState != 1) {
    LOG_ERROR(errMsg);
    return false;
  }
  ...
  if (eepromValuesNew.flowProfileState != 0 &&
      eepromValuesNew.flowProfileState != 1) {
    LOG_ERROR(errMsg);
    return false;
  }

  if (eepromValuesNew.homeOnShotFinish != 0 &&
      eepromValuesNew.homeOnShotFinish != 1) {
    LOG_ERROR(errMsg);
    return false;
  }
  ...
  if (eepromValuesNew.brewDeltaState != 0 &&
      eepromValuesNew.brewDeltaState != 1) {
    LOG_ERROR(errMsg);
    return false;
  }
  ...
  if (eepromValuesNew.powerLineFrequency != 50 &&
      eepromValuesNew.powerLineFrequency != 60) {
    LOG_ERROR(errMsg);
    return false;
  }
  ...
  eepromMetadata.versionTimestampXOR =
      eepromMetadata.timestamp ^ eepromMetadata.version;
...
void eepromInit(void) {
  ...
  if (version < EEPROM_DATA_VERSION &&
      legacyEepromDataLoaders[version] != NULL) {
  ...
  if (!readSuccess) {
    LOG_ERROR("SECU_CHECK FAILED! Applying defaults! eepromMetadata.version=%d",
              version);
    eepromMetadata.values = getEepromDefaults();
  }
